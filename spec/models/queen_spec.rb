...
RSpec.describe Queen, type: :model do
  ...
  describe 'move validation' do
    it '#in_queen_range returns true if horizontal move is in queen\'s range' do
      game = FactoryBot.create(:game)
      queen = FactoryBot.build(:queen, game_id: game.id)
      result = queen.send(:in_queen_range?, 8, 1) # if method is private
      # if not private: result = queen.in_queen_range?(4, 1, 5, 1)
      expect(result).to eq true
    end

    it '#in_queen_range returns true if vertical move is in queen\'s range' do
      game = FactoryBot.create(:game)
      queen = FactoryBot.build(:queen, game_id: game.id)
      result = queen.send(:in_queen_range?, 5, 5)
      expect(result).to eq true
    end

    it '#in_queen_range returns true if diagonal move is in queen\'s range' do
      game = FactoryBot.create(:game)
      queen = FactoryBot.build(:queen, game_id: game.id)
      result = queen.send(:in_queen_range?, 7, 3)
      expect(result).to eq true
    end

    it '#in_queen_range returns false if move is not in queen\'s range' do
      game = FactoryBot.create(:game)
      queen = FactoryBot.build(:queen, game_id: game.id)
      result = queen.send(:in_queen_range?, 8, 2)
      expect(result).to eq false
    end

    it '#is_queen_move_valid? returns true if queen\'s move is valid' do
      game = FactoryBot.create(:game)
      queen = FactoryBot.build(:queen, game_id: game.id)
      result = queen.is_queen_move_valid?(5, 2)
      expect(result).to eq true
    end

    it '#is_queen_move_valid? returns false if queen\'s move is off the board' do
      game = FactoryBot.create(:game)
      queen = FactoryBot.build(:queen, game_id: game.id)
      result = queen.is_queen_move_valid?(4, 0)
      expect(result).to eq false
    end

    it '#is_queen_move_valid? returns false if queen\'s move is out of range for queen' do
      game = FactoryBot.create(:game)
      queen = FactoryBot.build(:queen, game_id: game.id)
      result = queen.is_queen_move_valid?(5, 0)
      expect(result).to eq false
    end
  end
