...
RSpec.describe Pawn, type: :model do
  ...
  describe 'promotion' do
    it 'is_promotable? returns pawn.icon if the white pawn can be promoted' do
      pawn = FactoryBot.build(:pawn, color: 'white', y: 8)
      pawn.is_promotable?
      expect(pawn.icon).to eq '#9813'
    end
    ...
    it 'is_promotable? returns pawn.icon if the black pawn can be promoted' do
      pawn = FactoryBot.build(:pawn, color: 'black', y: 1)
      pawn.is_promotable?
      expect(pawn.icon).to eq '#9819'
    end
